...
type AttendanceStatus = 'P' | 'A' | 'AL' | 'SL' | 'PL' | 'WA';
type AttendanceGrid = Record<string, AttendanceStatus>;
type NotesMap = Record<number, string>;

const STATUS_CYCLE: AttendanceStatus[] = ['A', 'P', 'AL', 'SL', 'PL', 'WA'];
const STATUS_LABEL: Record<AttendanceStatus, string> = {
  P: 'Present', A: 'Absent', AL: 'Annual Leave', SL: 'Sick Leave', PL: 'Parental Leave', WA: 'Work Assignment',
};
const STATUS_BG: Record<AttendanceStatus, string> = {
  P: '#2d6a4f', A: '#f0f0f0', AL: '#1d4ed8', SL: '#b45309', PL: '#7c3aed', WA: '#c2410c',
};
const STATUS_TEXT: Record<AttendanceStatus, string> = {
  P: '#fff', A: '#999', AL: '#fff', SL: '#fff', PL: '#fff', WA: '#fff',
};
...
export default function AttendanceScreen() {
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    if (workers.some(w => ['P','AL','SL','PL','WA'].includes(grid[gridKey(w.id, d)] ?? 'A'))) {
      markedDays.add(d);
    }
  }
